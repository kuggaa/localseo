...
echo "<h3>For Search is  ".$forCustomSearch."</h3>";
$CSR   =   customsearch2($forCustomSearch);
//printarray($CSR);
...
for($i=0;$i<10;$i++){
    if($i!=0) // for next pages
        $CSR   =   customsearch2($forCustomSearch,($i*10+1),10);
...
foreach($CSR -> items as $Results){ // Go through all results 
    ...
    $mystring = $htmlSnippet = $Results -> snippet;
    $findstate   = $state;
    ...
    foreach($businessArray as $Each){  // Means if in any website in description the full given business name is found, it means that is the link we need
       echo "html snippet is <span style='color:green;'>".$htmlSnippet."</span> and each is <span style='color:green;'>".$Each."</span>";
        if(strpos($htmlSnippet,$Each) === false){
            echo "....strpos is false<br/>";
               $flag = false;
               continue;
        }
        else{
            echo "....strpos is true<br/>";
        }
    }
    
    if($Results -> displayLink == 'www.yellowbook.com'){
        printArray($businessArray);
        echo "flag is ".$flag." and businessname is ".$findbusnesName;
//        die;
    }
